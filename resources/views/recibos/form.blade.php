...
<div class="col-md-3">
  <div class="input-group input-group-sm">
    <input type="text" class="form-control form-control-sm" id="numero" placeholder="Número..." required>
...
/* ===== DÍAS BASE-30 (UX) ===== */
function diasMesBase30(fechaAfiliacion, fechaRecibo, fechaRetiro=null) {
  if (!fechaAfiliacion || !fechaRecibo) return 0;

  const fRec = parseLocalDate(fechaRecibo);
...
async function solicitarPreview() {
  ...
  const payload = {
    ...
    novedad: document.getElementById('novedad').value || 'Ingreso',
...
document.getElementById('btnBuscar').addEventListener('click', async () => {
  ...
  try {
    const { data } = await axios.get(`/recibos/buscar-usuario/${numero}`);
    usuarioCargado = data;
    ...
    solicitarPreview(); // 👈 primer cálculo real
